...
// TODO: adjust endpoint/payload to Dial's documented format once the
// curl sample from the Dial dashboard is wired in.
async function callDial({ to, prompt }) {
  const url = process.env.DIAL_API_URL;
  ...
  if (!url || !key) {
    return {
      simulated: true,
      message:
        "Demo mode: DIAL_API_URL / DIAL_API_KEY not set, no real call placed.",
      to,
      prompt,
    };
  }
