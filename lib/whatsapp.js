...
async function sendMessage({ to, message, channel }) {
  ...
  // An explicit channel (e.g. replying on the channel a message came in on)
  // overrides the env default.
  if (channel || process.env.DIAL_MESSAGE_CHANNEL) {
    payload.channel = channel || process.env.DIAL_MESSAGE_CHANNEL;
  }
